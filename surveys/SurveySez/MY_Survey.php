...
class MY_Survey extends Survey
{
	...
	{
    $myReturn = "";
    $sql ="select ResponseID, SurveyID , DateAdded from sp17_responses where ResponseID=$id";
    ...
    {#records exist - process
    	if($myPager->showTotal()==20){$itemz = "response";}else{$itemz = "responses";}  //deal with plural
      ...
      $myReturn.='
      <tr>
        <td><a href="' . VIRTUAL_PATH . 'surveys/response_view.php?id=' . (int)$row['ResponseID'] . '">
        ' . dbOut($row['DateAdded']) . ' </a></td>
      </tr>
      ';
    	}
      $myReturn.='
