...
class Response extends Survey
{
    ...
    function __construct($id)
	{
		...
		$selSQL = sprintf("select SurveyID, DateAdded from " . PREFIX . "responses where ResponseID =%d",$this->ResponseID);
		$result = mysqli_query($iConn,$selSQL) or die(trigger_error(mysqli_error($iConn), E_USER_ERROR));
		...
		{# Questions must exist for this survey, if we are to proceed
				...
				{
					$selSQL = sprintf("select AnswerID, QuestionID, RQID from " . PREFIX . "responses_answers where ResponseID=%d order by QuestionID asc",$this->ResponseID);
					$result = mysqli_query($iConn,$selSQL) or die(trigger_error(mysqli_error($iConn), E_USER_ERROR));
