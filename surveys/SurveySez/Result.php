 ...
 */
 ...
 class Result {
  ...
  public $SurveyID = 0;
	...
	{
    // $this->$myResult = (int)$id;
    // if($this->ResultID == 0){return FALSE;}
    ...
    // //
		...
		$sql = sprintf("select ResponseID from " . PREFIX . "responses where SurveyID =%d limit 0,5",$id);
		$result = mysqli_query($iConn,$sql) or die(trigger_error(mysqli_error($iConn), E_U1SER_ERROR));
  ...
  // function showTallies()
  // {# v5: Only one response now - no calculations done here!
  //   foreach($this->aResponse[0]->getaQuestions() as $question)
  //   {#loop through questions to reveal answers
  //     echo "<b>" . $question->Number . ") ";
  //     echo $question->Text . "</b> ";
  //     echo '<em>(' . $question->Description . ')</em> ';
  //     foreach($question->aAnswer as $answer)
  //     {#loop through to show totals for chosen answers (tallies)
  //         echo "<br />" $answer->Text .": <b>";
  //           foreach (($this->aTally as $tally) {
  //             #find match of Tally's AnswerID to current Answer
  //               if($answer->AnswerID == $tally->AnswerID)
  //               {
  //                 # show tally totals
  //                   echo  $tally->Total ."</b> ";
  //                   break;
	...
	{# v5: creates a bar graph effect by effecting background color of divs of varying lengths
		...
		{//loop through questions to reveal answers
			...
			{# loop through to show totals for chosen answers (tallies)
				...
				echo 'background:url(' . VIRTUAL_PATH . 'surveys/images/2px' . $aColors[$x] . '.gif) left repeat"></div></td>';
				$x += 1;
